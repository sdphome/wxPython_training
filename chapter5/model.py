...
class SimpeName(abstractmodel.AbstractModel):
...
class ModelExample(wx.Frame):
    ...
    def createCaptionedText(self, panel, label, pos):
        ...
        self.textFields[label] = wx.TextCtrl(panel, wx.NewId(), "", size=(100, -1), pos=textPos, style=wx.TE_READONLY) # 显示文本框

    def OnUpdate(self, model):
        self.textFielfs["First Name"].SetValue(model.first)
        self.textFields["Last Name"]. SetVaule(model.last)
    
    def OnFred(self, event):
        self.model.set("Fred", "Flintstone")

    def OnBarney(self, event):
        self.model.set("Barney", "Flintstone")

    def OnWilma(self, event):
        self.model.set("Wilma", "Rubble")
